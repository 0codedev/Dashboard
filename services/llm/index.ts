...
export const llmPipeline = async (options: GenerationOptions): Promise<string> => {
    ...
    if (includeFooter && !expectJson && !jsonSchema && usedModelId) {
        ...
        if (fallbackFrom && fallbackFrom !== 'none') {
             ...
             footerHtml = `\n\n<div class="text-[10px] text-slate-400 mt-4 pt-2 border-t border-slate-700/50 flex items-center gap-1"><span class="text-amber-500">⚠️</span> Fallback to <strong>${modelName}</strong> <span class="text-slate-500">(${fallbackName} unavailable)</span></div>`;
